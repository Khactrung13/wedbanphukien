...
<?php
	...
	$id=$_GET['id'];	
	$sql_probv = "SELECT * FROM tbl_baiviet WHERE tbl_baiviet.id_danhmucbaiviet ='$_GET[id]' 
...
?>
<h3 style="padding-left: 20px;font-size: 20px;margin-top:7px;margin-bottom: 5px;"> Danh mục bài viết : <?php echo $row_title['tendanhmuc_baiviet']?> </h3>
				<ul>
					<?php
					...
					<li class ="danhmucbaiviet">
						<a href="index.php?ql=baiviet&id=<?php echo $row_pro['id_baiviet'] ?>">
							...
							<p style="text-decoration: none;text-align: center;font-size: 20px;font-weight: bold;" class="danhmuc"><?php echo $row_pro['tenbaiviet'] ?></p>
				...
				<?php
					$sql_trang= mysqli_query($mysqli,"SELECT * FROM tbl_baiviet WHERE id_danhmucbaiviet = '$_GET[id]'");
				...
				<ul class="list_trang">
					...
					?>
							<li><a <?php if($i == $page){ 
								...
								?> 
						href="index.php?ql=danhmucbaiviet&id=<?php echo $id?>&trang=<?php echo $i?>"><?php  echo $i ?></a></li>
					<?php
						}
					?>
